...
public class SrsPublisher {
    ...
    public static byte[] sps = {103, 66, -128, 31, -38, 1, 64, 22, -23, 72, 40, 16, 16, 54, -123, 9, -88};
    public static byte[] pps = {104, -50, 6, -30};
    ...
    public void startPublish2(String rtmpUrl) {
        if (mFlvMuxer != null) {
            mFlvMuxer.start(rtmpUrl);
            mFlvMuxer.setVideoResolution(mEncoder.getOutputWidth(), mEncoder.getOutputHeight());
            // 开启推流发送记录值 0代表 推流首帧
            mFlvMuxer.setPublishTag(0);
        }
    }

    public void stopPublish2() {
        if (mFlvMuxer != null) {
            mFlvMuxer.stop();
        }
    }

    public void stopEncode2() {
        stopAudio();
        mCameraView.disableEncoding();
        mEncoder.stop();
    }
