...
//                Arrays.fill(h264_pps.array(), (byte) 0x00);
        ...
        public void writeVideoSample(final ByteBuffer bb, MediaCodec.BufferInfo bi) {
            ...
            if (i == 0 && SrsPublisher.sps != null && SrsPublisher.pps != null) {
                // 当i=0时 此时推流刚刚开启，将记录的sps 与 pps 写入
                // 视频推流流程：onEncodedAnnexbFrame - writeSampleData - writeVideoSample - writeH264SpsPps - writeRtmpPacket - flvTagCacheAdd - mFlvTagCache.add
                h264_sps_changed = true;
                h264_sps = ByteBuffer.wrap(SrsPublisher.sps);

                h264_pps_changed = true;
                h264_pps = ByteBuffer.wrap(SrsPublisher.pps);
                Log.e(TAG, "writeVideoSample pps: " + Arrays.toString(SrsPublisher.pps));
                Log.e(TAG, "writeVideoSample sps: " + Arrays.toString(SrsPublisher.sps));
